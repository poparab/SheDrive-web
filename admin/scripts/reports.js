...
filters.fields = [
  { type: 'daterange', key: 'date', label: 'Date range', fromKey: 'from', toKey: 'to' },
  {
    type: 'select',
    key: 'zoneId',
    label: 'Zone',
...
filters.actions = [
  {
    ...
    onClick: () => {
      ...
      downloadCsv(
        ...
        ['Metric', 'Value'],
        [
          ['Date from', query.from || 'All time'],
          ['Date to', query.to || 'All time'],
          ['Zone', zoneLabel],
          ['Completed trips', summary.completedTrips],
          ['Gross fares (EGP)', summary.grossFares.toFixed(2)],
          ['Platform commission (EGP)', summary.commission.toFixed(2)],
          ['Cancellation fees (EGP)', summary.cancellationFees.toFixed(2)],
          ['Net driver earnings (EGP)', summary.netDriverEarnings.toFixed(2)],
        ],
      );
    },
  },
...
function renderReconciliation(data) {
  ...
  host.append(
    moneyRow('Platform commission', formatEgp(data.commission)),
    moneyRow('Net driver earnings', formatEgp(data.netDriverEarnings)),
    moneyRow('Commission + net earnings', formatEgp(sum), true),
    moneyRow('Gross fares', formatEgp(gross)),
  );
...
async function load() {
  ...
  try {
    ...
    const meta = data.completedTrips === 0 ? 'No completed trips in this period' : '';
    ['card-trips', 'card-gross', 'card-commission', 'card-fees', 'card-net'].forEach((id) => {
      qs(`#${id}`).meta = meta;
    });
