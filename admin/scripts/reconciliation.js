 ...
 *
 * Recording a settlement and the full ledger live on balances.html
...
filters.actions = [
  {
    ...
    onClick: () => {
      ...
      downloadCsv(
        ...
        ['Trip date', 'Fare (EGP)', 'Commission (EGP)', 'Net earnings (EGP)'],
...
table.columns = [
  ...
  { key: 'fare', label: 'Fare (EGP)', numeric: true, render: (t) => formatEgp(t.fare.total) },
  {
    key: 'commission',
...
table.emptyState = {
  icon: '☐',
  heading: 'No completed trips in this period',
  message: 'This driver completed no trips in the selected date range.',
};
...
async function load() {
  ...
  try {
    ...
    qs('#card-balance').meta = 'Cash held by the driver, owed to the platform';

    const split = qs('#split-rows');
    split.textContent = '';
    split.append(
      moneyRow('Collected in cash', formatEgp(t.cashPortion)),
      moneyRow('Collected digitally', formatEgp(t.digitalPortion)),
      moneyRow('Gross fares', formatEgp(t.grossFares), true),
      moneyRow('Outstanding cash balance', formatEgp(t.outstandingCashBalance)),
    );
