 ...
 *
 * RENDER (v2): the map keeps every behaviour, class name and layer id it had —
 * only the chrome around it is the delivered kit's card:
 *
 *   <ad-map-panel class="ad-map">
 *     <div class="standard-card-theme2 white-bg ad-map__card">
 *       <div class="card-header">            ← only when panel-title is set
 *         <div class="card-title"><h5>…</h5></div>
 *         <div class="icon"><img src="vendor/icons/gps.svg"></div>
 *       </div>
 *       <div class="card-body ad-map__body">
 *         <div class="ad-map__toolbar">…kit buttons + .ad-map__legend…</div>
 *         <div class="ad-map__frame">
 *           <div class="ad-map__canvas"></div>   ← Mapbox owns this node
 *           <div class="ad-map__overlay"><p class="ad-state__message"></p></div>
 *         </div>
 *       </div>
 *     </div>
 *   </ad-map-panel>
 *
 * `ad-map*`, `ad-marker*` and `ad-popover*` class names are unchanged, so the
 * existing admin.css rules and every screen script keep working.
 *
 * Usage:
 ...
 *
 * Attributes:
 *   panel-title     — card header heading (literal text); omit it and no header
 *                     is rendered, which is what the screens that already carry
 *                     an .ad-section__header above the map need.
 *   panel-title-key — the same heading as a translation key (preferred).
 *   panel-icon      — kit icon file for the header, default `gps.svg`.
 *
 * Every string the component itself supplies — the four overlay messages, the
 * two boundary-draw errors, and the map / toolbar / legend `aria-label`s —
 * comes from `t()`. Toolbar button labels and legend item labels are passed in
 * already translated by the screen.
 *
 * Children with data-slot="toolbar" render in the toolbar row and are given the
 * kit's small button classes when they carry no kit variant of their own.
...
/** True for a value with no Arabic letters — an id, plate, fare or timestamp. */
function isLatinRun(value) {
...
/**
 * Toolbar controls are the kit's small buttons. A slotted control that already
 * names a kit variant is left alone; anything else (including the legacy
 * `.btn--ghost` vocabulary) is given the neutral outline treatment.
 */
function skinToolbarControl(node) {
  ...
  const hasKitVariant = Array.from(node.classList).some(
...
class AdMapPanel extends HTMLElement {
  ...
  build() {
    ...
    this._card.className = 'standard-card-theme2 white-bg ad-map__card';

    const titleKey = this.getAttribute('panel-title-key');
    const heading = titleKey ? t(titleKey) : this.getAttribute('panel-title');
    if (heading) {
      ...
      iconImg.src = iconUrl(this.getAttribute('panel-icon') ?? 'gps.svg');
      iconImg.alt = '';
    ...
    this._body.className = 'card-body ad-map__body';

    this._toolbar = document.createElement('div');
    this._toolbar.className = 'ad-map__toolbar';
    this._toolbar.setAttribute('role', 'group');
    this._toolbar.setAttribute('aria-label', t('map.toolbar'));
    ...
    this._canvas.setAttribute('aria-label', t('map.region'));
    ...
    this._body.append(this._toolbar, this._frame);
    ...
    passthrough.forEach((node) => {
      if (node.nodeType === Node.ELEMENT_NODE && node.dataset?.slot === 'toolbar') {
        this._toolbar.appendChild(skinToolbarControl(node));
      }
    });
  ...
  addToolbarButton(label, onClick, { variant = 'btn-light-outline', size = 'btn--sm' } = {}) {
    ...
    button.type = 'button';
    button.className = `btn ${variant} ${size}`.trim();
  ...
  mount({ center = DEFAULT_MAP_CENTER, zoom = 10.5, tall = false } = {}) {
    ...
    if (typeof mapboxgl === 'undefined') {
      this.setEmpty(true, t('map.libraryMissing'));
    ...
    if (typeof mapboxgl.supported === 'function' && !mapboxgl.supported()) {
      this.setEmpty(true, t('map.webglUnavailable'));
      return Promise.resolve(null);
    }
    ...
    if (!this._map) {
      this.setEmpty(true, t('map.noToken'));
      return Promise.resolve(null);
    }
    ...
    return new Promise((resolve) => {
      ...
      const timer = window.setTimeout(() => {
        if (settled) return;
        settled = true;
        this.setEmpty(true, t('map.tilesFailed'));
        resolve(null);
      }, MOUNT_TIMEOUT_MS);
    });
  }
  ...
  showPopover(coordinates, marker) {
    ...
    (marker.popover?.rows ?? []).forEach(([label, value]) => {
      ...
      // Popover values are often ids, plate numbers, fares or ETAs — Latin runs
      // the bidi algorithm would re-order inside an Arabic popover. Values that
      // are themselves Arabic (a name, a zone) are left to flow normally.
      if (isLatinRun(text)) val.classList.add('ad-ltr');
  ...
  finishDraw() {
    if (this._drawPoints.length < 3) {
      return { polygon: null, error: t('map.drawTooFewPoints') };
    }
    if (selfIntersects(this._drawPoints)) {
      return { polygon: null, error: t('map.drawSelfIntersects') };
    }
